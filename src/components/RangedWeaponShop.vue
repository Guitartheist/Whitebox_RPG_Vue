<template>
  <div v-if="show_shop == 'ranged_weapons'">
  <h1>Ranged Weapons Shop</h1>
  <table>
  <tr>
	<td>Name</td>
	<td>Damage</td>
...
export default {
  ...
  methods: {
	...
	Buy(event) {
		...
		if (confirm("Buy " + this.weapon.name + " for " + this.weapon.cost_gp + " gold?")){
			this.$emit('BuyRangedWeapon', event.target.id);
		}
	}
  },
  ...
  watch: {
	show_shop() {
		if (this.show_shop=='ranged_weapons') {
			HTTP.get('RangedWeaponList')
				.then(response => {
				// JSON responses are automatically parsed.
				this.weapons = response.data
